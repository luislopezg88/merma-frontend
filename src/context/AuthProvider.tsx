...
import type { AuthResponse, User } from '../configs/types'
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  function saveUser(userData: AuthResponse) {
    ...
    //Store
    const store = userData.body.user
    sessionStorage.setItem('session-merma', JSON.stringify(store))
  }
  ...
  function getRefreshToken() {
    ...
    if (token) {
      ...
      setRefreshToken(refreshToken)
      //sesion
      const session: any = sessionStorage.getItem('session-merma')
      const parseado = JSON.parse(session)
